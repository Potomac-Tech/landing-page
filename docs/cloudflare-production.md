...
Only the apex and `www` are attached to this Worker. Do not change email records, other subdomains, or the original private Sites deployment.
...
To return website traffic to GitHub Pages, remove only this Worker's two custom-domain attachments and restore the previously observed DNS targets in Cloudflare DNS:
...
- No apex AAAA record was present before migration. Use a 300-second TTL for the restored website records.

Keep the Pages custom-domain setting `potomacdb.com`. Leave MX/TXT, all other DNS records, the branch preview and D1 data unchanged. DNS restoration requires a Cloudflare DNS-authorized session; the existing Wrangler OAuth session can manage Workers but cannot directly read/edit DNS records.
