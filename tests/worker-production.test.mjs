...
import {
  ...
  productionRequest,
  productionResponse,
} from '../lib/worker-production.ts';

test('production canonical redirects cannot be redirected to a path-supplied host', () => {
  const result = productionRedirect(
    new Request('https://www.potomacdb.com//evil.example/path?q=1'),
  );
  assert.equal(result.status, 308);
  assert.equal(
    result.headers.get('location'),
    'https://potomacdb.com//evil.example/path?q=1',
  );
  assert.equal(
    productionRedirect(new Request('https://unknown.example/')).status,
    421,
  );
  assert.equal(
    productionRedirect(new Request('https://potomacdb.com/')),
    undefined,
  );
});

test('production requests remove forged identity and forwarding but preserve POST bodies', async () => {
  const clean = productionRequest(
    new Request('https://potomacdb.com/api/inquiries', {
      method: 'POST',
      body: '{"test":true}',
      headers: {
...
test('only successful public production pages are indexable; security and Vary survive', () => {
  for (const [url, status, noindex] of [
    ...
    const result = productionResponse(
      new Response('ok', { status, headers: { Vary: 'RSC' } }),
      new Request(url),
    );
